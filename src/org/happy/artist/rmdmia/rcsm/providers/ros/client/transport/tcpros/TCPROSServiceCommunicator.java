...
/** TCPROSServiceCommunicator.java TCPROS Services client.
 ...
 */
public class TCPROSServiceCommunicator implements CommunicationSenderInterface, CommunicationReceiverInterface
{
    ...
    public TCPROSServiceCommunicator(String initializerMessageHandlerClassname,String messageHandlerClassname, String tunerMessageHandlerClassname, String topic, boolean isTopic, ROSNode rosNode) {
    ...
    public void initialize(String hostname, int port, int blockSize, boolean tcp_nodelay, boolean keep_alive, String threadName) throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException
    {
        ...
        {
            private TCPDataProcessor processor = new TCPDataProcessor(lastBlockSize,TCPROSServiceCommunicator.this.initializerMessageHandlerClassname,TCPROSServiceCommunicator.this.messageHandlerClassname, TCPROSServiceCommunicator.this.tunerMessageHandlerClassname, TCPROSServiceCommunicator.this.threadName, TCPROSServiceCommunicator.this.topic, TCPROSServiceCommunicator.this.isTopic,TCPROSServiceCommunicator.this,TCPROSServiceCommunicator.this.socket,0);

            @Override
            public void run() 
            {
                // Set the TCPROSServiceCommunicator messageHandlerController
                TCPROSServiceCommunicator.this.messageHandlerController=(MessageHandlerControllerInterface)processor;
                // Set the TCPROSServiceCommunicator isRunning to true.
                TCPROSServiceCommunicator.this.isRunning=true;
                while(isRunning)
                {
                    try
                    {
                    ...
                    {
                        TCPROSServiceCommunicator.this.isRunning=false;
                    }                    
                    catch(Exception e)
                    {
                        System.out.println("Exception in thread: " + TCPROSServiceCommunicator.this.threadName);
                        e.printStackTrace();
                    }
                }
            }
...
// TODO: Implement thread on start mechanism & other TopicRegistry variable settings pre-instantiation. Verify if it includes connect method as well. Start method should be called via the Array after initialization and not in initialization.           
        ...
        {
            throw new RCSMException("Unknown Host called in start method of TCPROSServiceCommunicator. hostname:" + hostname + ":" + String.valueOf(port),ex);
        }        
        catch (IOException ex) 
        {         
            throw new RCSMException("IOException in start method of TCPROSServiceCommunicator.",ex);
        }
    }
    ...
    public void connect() throws RCSMException 
    {
        try 
        {
            reinitialize();
            start();
            send(rosNode.srvMessages[rosNode.getServiceIndex(topic)].getMessage());
        } 
        catch (IOException ex) 
        {
...
//        this.finalByteArray=null;
        ...
        // Set the TCPROSServiceCommunicator messageHandlerController to null
    ...
    /** only call send, after TCPROSServiceCommunicator initialize method is called at once at Object initialization time. Keep in mind this data array should match the length of the packetLength. */
