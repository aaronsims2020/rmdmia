 ...
 * 
 */
public class ROSTurtlebotController extends ROCIProvider
{
    ...
    // turtlesim movements.
    private static byte[] GAMEPAD_UP=Movements3.getUpMovement();
    private static byte[] GAMEPAD_DOWN=Movements3.getDownMovement();
    private static byte[] GAMEPAD_LEFT=Movements3.getLeftMovement();
    private static byte[] GAMEPAD_RIGHT=Movements3.getRightMovement();
    ...
    private Logger logger = Logger.getLogger(ROSTurtlebotController.class.getName());  
    // set instance in constructor for message callback to reference.
    private static ROSTurtlebotController instance=null;
    public ROSTurtlebotController()
    {
        this.instance=this;
    }
    
    /** return reference to constructed ROSTurtlesimController. */
    public static ROSTurtlebotController getInstance()
    {
        return instance;
    }
...
//                System.out.println("Turtle Sender Topic threadName: " + ((TCPROSPublisherCommunicator)sender).threadName);
                ...
                public void run()
                {
                    try 
                    {
                        if(ROSTurtlebotController.this.sender!=null)
                        {
      //                      System.out.println("calling startController on /turtle1/cmd_vel");
                            startController(ROSTurtlebotController.this.sender);
                        }
                        else
                        {
                        Logger.getLogger(ROSTurtlebotController.class.getName()).log(Level.INFO, "Turtlesim Controller could not send message due to the Publisher not being initialized.");
                            
                        }
                    } catch (IOException ex) 
                    {
                        Logger.getLogger(ROSTurtlebotController.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
       });
                thread.start();
 ...
 //           this.monitor = new ConnectionMonitor(tableData, tableColumnNames, rosNode.getCallerID(), rosNode.getSubscriberMessageManager(), rosNode, rosNode.getProperties(), rosNode.getPropertiesFilePath());                   
    ...
    private final static byte[] MOTOR_ON=HexToBytes.hexToBytes("0100000001");
    private final static byte[] MOTOR_OFF=HexToBytes.hexToBytes("0100000000");
  ...
  //       instruction.state=instruction.ON;
         ...
         motor_state_sender.send(MOTOR_ON);
...
//         instruction.state=instruction.OFF;
         ...
         motor_state_sender.send(MOTOR_OFF);        
        // motor_power_handler.send(instruction);
        ...
        while(true) 
        {
            ...
            for(int i=0;i<controllers.length;i++) 
            {
              // System.out.println("Polling...");
               ...
               {
                  // System.out.println(cname+event.getValue());                    
