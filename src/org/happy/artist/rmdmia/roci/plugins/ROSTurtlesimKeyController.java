...
import org.happy.artist.rmdmia.rcsm.providers.ros.ROSTopicStates;
import org.happy.artist.rmdmia.rcsm.providers.ros.ROSXMLRPCException;
 ...
 * 
 */
public class ROSTurtlesimKeyController extends ROCIProvider
{
    ...
    private Logger logger = Logger.getLogger(ROSTurtlesimKeyController.class.getName());  
    // set instance in constructor for message callback to reference.
    private static ROSTurtlesimKeyController instance=null;
    public ROSTurtlesimKeyController()
    {
        this.instance=this;
    }
    
    /** return reference to constructed ROSTurtlesimKeyController. */
    public static ROSTurtlesimKeyController getInstance()
    {
        return instance;
    }
...
//                System.out.println("Turtle Sender Topic threadName: " + ((TCPROSPublisherCommunicator)sender).threadName);
                ...
                public void run()
                {
                    try 
                    {
                        if(ROSTurtlesimKeyController.this.sender!=null)
                        {
      //                      System.out.println("calling startController on /turtle1/cmd_vel");
                            startController(ROSTurtlesimKeyController.this.sender);
                        }
                        else
                        {
                        Logger.getLogger(ROSTurtlesimKeyController.class.getName()).log(Level.INFO, "Turtlesim Controller could not send message due to the Publisher not being initialized.");
                        // TODO: Remove
                        startController(ROSTurtlesimKeyController.this.sender);
                        }
                    } catch (IOException ex) 
                    {
                        Logger.getLogger(ROSTurtlesimKeyController.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
       });
                thread.start();
 ...
 //           this.monitor = new ConnectionMonitor(tableData, tableColumnNames, rosNode.getCallerID(), rosNode.getSubscriberMessageManager(), rosNode, rosNode.getProperties(), rosNode.getPropertiesFilePath());                   
    ...
    public void startController(CommunicationSenderInterface sender) throws IOException
    {
        ...
        JFrame frame = new JFrame("Turtlesim Key Controller Demo");
