...
// start method variables.    
    ...
    public void start(OutputStream os, int COMPRESSION_TYPE) throws IOException
    {
        this.os=os;
        ...
        if(COMPRESSION_TYPE==BagOutputStream.CHUNK_COMPRESSION_BZ2)
        {
...
;
        try
        {
            os.write(HexToBytes.hexToBytes(sb.toString()));
        }
        catch(IOException e)
        {
            throw e;
    ...
    public synchronized void addConnectionHeader(char[] topic, int conn, char[] connection_header_hex) throws IOException
    {
        ...
        connection_header_sb.append(createHexField(CONNECTION_TOPIC_KEY, BagOutputStream.stringToHex(String.valueOf(topic))));       
        ...
        connection_header_sb.append(createHexField(CONNECTION_CONN_KEY, BagOutputStream.bytesToHex(conn_array)));
        ...
        connection_header_sb.insert(0,BagOutputStream.bytesToHex(conn_array)); 
        // Append Connection Message Header Data to Connection Header 
        connection_header_sb.append(connection_header_hex);        
        os.write(HexToBytes.hexToBytes(connection_header_sb.toString()));
    }
    ...
    public synchronized void addMessage(long time, int conn, char[] message_data_hex) throws IOException
    {
        ...
        message_data_header_sb.append(createHexField(MESSAGE_DATA_TIME_KEY, BagOutputStream.bytesToHex(message_data_time_array)));
        // conn header field
        ...
        message_data_header_sb.append(createHexField(MESSAGE_DATA_CONN_KEY, BagOutputStream.bytesToHex(message_data_conn_array)));
        ...
        // insert the header length into the header
        message_data_header_sb.insert(0,BagOutputStream.bytesToHex(message_data_conn_array)); 
        // append message_data
        message_data_header_sb.append(message_data_hex);
        os.write(HexToBytes.hexToBytes(message_data_header_sb.toString()));
    } 
    ...
    private String createBagHeader(long index_pos_length, int conn_count, int chunk_count)
    {
        ...
        bag_header_sb.append(createHexField(INDEX_POS_LENGTH_KEY, BagOutputStream.bytesToHex(index_pos_length_array)));
        ...
        bag_header_sb.append(createHexField(CONN_COUNT_KEY, BagOutputStream.bytesToHex(connection_count_array)));
        ...
        bag_header_sb.append(createHexField(CHUNK_COUNT_KEY, BagOutputStream.bytesToHex(connection_count_array)));
        ...
        // insert the header length into the header
        bag_header_sb.insert(0,BagOutputStream.bytesToHex(connection_count_array));      
    ...
    private void padBagHeader() 
    {
        ...
        bag_header_sb.append(BagOutputStream.bytesToHex(padding_length_bytes));                
        // append padding
    ...
    private String createChunkHeader(int compression_type, int size)
    {
        ...
        {
            // BZ2 Compression
            chunk_header_sb.append(createHexField(CHUNK_COMPRESSION_KEY, HEX_CHUNK_COMPRESSION_BZ2)); 
        }
        else 
        {
            // No compression
...
// TODO: Handle Size field based on compression type.        
        ...
        chunk_header_sb.append(createHexField(CHUNK_SIZE_KEY, BagOutputStream.bytesToHex(chunk_size_array)));
        // Re-using sb_header_length (this may need to be separated in the future... If it is called in an non-synchronous manner.
        ...
        // insert the header length into the header
        chunk_header_sb.insert(0,BagOutputStream.bytesToHex(chunk_size_array));
    ...
    private String createHexField(char[] hex_field_name, char[] hex_field_value)
    {
        ...
        hex.insert(0, BagOutputStream.bytesToHex(field_length_bytes));
        return hex.toString();
    }
    ...
    private String createHexField(char[] hex_field_name, String hex_field_value)
    {
        ...
        str_hex.insert(0, BagOutputStream.bytesToHex(str_field_length_bytes));
    ...
    {
        BagOutputStream bag = new BagOutputStream();
        System.out.println("Bag Header: ".concat(bag.createBagHeader(((long)878657565), 256, 7))); 
    // TODO: Implement and test with BZ2 support.
        System.out.println("Chunk Header: ".concat(bag.createChunkHeader(BagOutputStream.CHUNK_COMPRESSION_NONE, 0)));
    }      
